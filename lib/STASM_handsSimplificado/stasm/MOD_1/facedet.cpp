    ...
    void*)                       // in: unused (func signature compatibility)
{
    ...
    OpenDetector(facedet_g, "hands_toy.xml",  datadir);
}
    ...
    int          minwidth) // in: as percent of img width
{
    ...
    CV_Assert(minwidth >= 1 && minwidth <= 100);
		...
		minwidth = 50; 	// Force minwidth to reduce false positives with toy detector. In other words,
										// the minimun width of a hand must be 50% of the image width.
